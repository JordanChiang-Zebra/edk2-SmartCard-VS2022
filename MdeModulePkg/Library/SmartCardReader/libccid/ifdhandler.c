	...
	LPSTR lpcDevice, void *Channel)
{
	...
	*CcidSlots[reader_index].pcATRBuffer = '\0';
	...
	if (lpcDevice)
		...
		ret = OpenPort(reader_index, (void *)Channel);
	...
	else
	{
		...
		ccid_descriptor->readTimeout = (unsigned int)(ccid_descriptor->readTimeout * 100.0 / DEFAULT_COM_READ_TIMEOUT);
...
EXTERNAL RESPONSECODE IFDHCreateChannelByName(DWORD Lun, LPSTR lpcDevice)
{
	...
	return CreateChannelByNameOrChannel(Lun, lpcDevice, NULL);
}

EXTERNAL RESPONSECODE IFDHCreateChannel(DWORD Lun, void *Channel)
{
	/*
	...
	PDWORD pdwBytesReturned)
{
	...
	*pdwBytesReturned = 0;
	...
	if (IOCTL_FEATURE_IFD_PIN_PROPERTIES == dwControlCode)
	{
		...
		set_U16(&caps -> wLcdLayout, (UINT16)(ccid_descriptor -> wLcdLayout));
		...
		caps -> bEntryValidationCondition = (unsigned char)validation;
	...
	if (IOCTL_FEATURE_GET_TLV_PROPERTIES == dwControlCode)
	{
		...
		/* wIdVendor */
		{
			...
			RxBuffer[p++] = (unsigned char)(idVendor >> 8);
		}
		...
		/* wIdProduct */
		{
			...
			RxBuffer[p++] = (unsigned char)(idProduct >> 8);
		}
	...
	int clock_frequency)
{
	...
	/* ISO in */
	t  = (unsigned int) (261 * EGT + (3 + 3) * WWT);
	if (timeout < t)
		timeout = t;

	/* ISO out */
	t = (unsigned int) (5 * EGT + (1 + 259) * WWT);
	if (timeout < t)
		timeout = t;
	...
	int BWI, int CWI, int clock_frequency)
{
	...
	timeout = (unsigned int) (260*EGT + BWT + 260*CWT);
